...
const OrderDetails = () => {
  ...
  const downloadInvoice = () => {
    ...
    doc.text(`Address: ${shipping?.address}, ${shipping?.city} - ${shipping?.zip}`, 20, 130);

    doc.text("Order Summary", 20, 150);
    let y = 160;
    cart?.forEach((item) => {
      doc.text(`${item.title || item.name} x ${item.quantity} - ₹${item.price * item.quantity}`, 20, y);
      y += 10;
    });
  ...
  if (!orderId) {
    return <p className="text-center py-12 text-gray-500">Order not found</p>;
  }
  ...
  return (
    <div className="min-h-screen bg-pink-50 px-4 py-10 font-poppins">
      <div className="max-w-4xl mx-auto bg-white rounded-2xl shadow-xl p-6 sm:p-10">
        ...
        {/* Order Info */}
        <div className="mb-6">
          <h3 className="text-lg font-semibold text-pink-600 mb-2">Order Information</h3>
        ...
        {/* Shipping Info */}
        <div className="mb-6">
          <h3 className="text-lg font-semibold text-pink-600 mb-2">Shipping Details</h3>
          ...
          <p><span className="font-medium">Address:</span> {shipping?.address}, {shipping?.city} - {shipping?.zip}</p>
        </div>

        {/* Order Summary */}
        <div className="mb-6">
          <h3 className="text-lg font-semibold text-pink-600 mb-3">Order Summary</h3>
          {cart?.map((item, index) => (
            <div
              key={index}
            ...
            >
              <div className="flex items-center gap-4">
                ...
                />
                <div>
                  <p className="font-medium">{item.title || item.name} × {item.quantity}</p>
          ...
          <p className="text-gray-700 mt-2">
            <span className="font-medium">Estimated Delivery:</span> {estimatedDate.toDateString()}
          </p>
        </div>
